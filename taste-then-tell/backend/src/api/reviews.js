...
router.delete(
  ...
  errorWrap(async (req, res) => {
    ...
    db.query(QUERY, (err, results) => {
      ...
      if (results.affectedRows == 0) {
        return sendSuccess(res, `No review deleted because no review exists with review_id: ${review_id}`, results);
      }
...
router.get(
  '/average_ratings/',
  errorWrap(async (req, res) => {
    const QUERY = 'SELECT u.name, dh.name, AVG(r.rating) as avgRating FROM Reviews r NATURAL JOIN Dining_Halls dh JOIN Universities u USING(university_id) GROUP BY dh.dining_hall_id ORDER BY avgRating DESC';
    db.query(QUERY, (err, results) => {
      console.log(QUERY);
      if (err) {
        return res.send(err);
      ...
      sendSuccess(res, 'Successfully returned average rating for all foods within all dining halls', results);
    });
  }),
);
